...
extraeSigloNegSolar <- function(yearbase){
  ...
  # URL de la página
  url <- paste0("https://eclipsewise.com/solar/SEcatalog/SE-",
...
extraeSigloPosSolar <- function(yearbase){
...
url <- paste0("https://eclipsewise.com/solar/SEcatalog/SE-0099-0000.html")
...
for (i in seq(100, 2900, 100)){
  ...
  df <- rbind(df, extraeSigloNegSolar(i))  
  
}

################################################################################
#                                                                              #
#                   Paso 3: Se extrae el Siglo I AC                            #
#                                                                              #
...
for (i in seq(1, 2901, 100)){
  ...
  df <- rbind(df, extraeSigloPosSolar(i))  
...
df_0 <- df[c(249:251),] #Información del siglo I BC o año 0 según criterio Fred
df <- df[c(-251:-249),] #Resto de la información
...
SolarEclipsesDates <- rbind(dfNeg, dfPos)

#Se crean nombres de columnas adecuados para ser tratados por apps

names(SolarEclipsesDates) <- c("HistDate", "TDGreatestEclipse", "ΔT",
                              "ΔTSigma", "LunaNum", "SarosNum",
                              "EclType", "QLE", "Gamma", "EclMag",
                              "Lat", "Long", "SunAlt","PatWidthKm", 
                              "CentralDur")

save(SolarEclipsesDates, file = "SolarEclipsesDates.rda")